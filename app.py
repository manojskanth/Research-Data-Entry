...
# --- 2. GOOGLE AUTH (The Hardened Loader) ---
def get_google_credentials():
    # Uses literal newline characters to ensure PEM compatibility
...
# ... [The rest of your existing logic for document gen and sheet appending goes here] ...

# --- 4. APP UI ---
st.set_page_config(page_title="St. Mary's Integrated Portal", layout="wide")

# ... [Login, Tabs, Admin, Footer logic] ...import streamlit as st
import datetime
from google.oauth2 import service_account
from googleapiclient.discovery import build
from googleapiclient.http import MediaIoBaseUpload
...
# ... [Keep your existing upload_file_to_drive, append_and_sort_sheet_by_department, and build_monthly_word_document logic here] ...
...
with tabs[0]: # Submission
    if not st.session_state.admin_enabled and st.session_state.logged_email != "research@stmaryscollege.in":
        ...
        collab_names = st.text_input("Collaborator Names (Mandatory)") if collab else ""
        upload = st.file_uploader("Upload Verification (Mandatory)")
        if st.form_submit_button("Submit"):
            if collab and not collab_names: st.error("Collab names required!")
...
with tabs[1]: # Generator
    ...
    # ... [Keep your original generator logic here] ...
