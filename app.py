...
def get_google_credentials():
    ...
    # Ensure newline format is absolute
...
with tabs[0]:
    if not st.session_state.admin_enabled and st.session_state.email != "research@stmaryscollege.in":
        ...
        cat = st.selectbox("Select Entry Classification", ["--- Select ---", "🔬 Research Database", "🏆 Faculty Profiles", "👥 Student Contributions"])
        if cat == "🔬 Research Database":
            with st.form("research_db_form", clear_on_submit=True):
                ...
                if st.form_submit_button("Submit"):
                    if collab and not collab_names: st.error("Names required!")
...
with tabs[1]:
    ...
    view_dept, view_month, view_year = c1.selectbox("Dept", DEPARTMENTS), c2.selectbox("Month", MONTHS), c3.selectbox("Year", ACADEMIC_YEARS)
    if st.button("🏗️ Construct Automated Monthly Document Package"):
        with st.spinner("Assembling document..."):
            ...
            st.download_button("📥 Download", data=bytes_data, file_name="Report.docx")
