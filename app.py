...
\
st.set_page_config(page_title="Faculty Portal", page_icon="\uc0\u55357 \u56541 ", layout="wide")\
\
# --- INTERFACE ROUTING: LOGIN WALL ---\
if not st.session_state.authenticated:\
    st.markdown("<h2 style='text-align: center;'>\uc0\u55357 \u56592  St. Mary's Secure Research Gateway</h2>", unsafe_with_html=True)\
    st.markdown("<p style='text-align: center;'>Authorized Faculty Login Panel</p>", unsafe_with_html=True)\
    \
    col_l1, col_l2, col_l3 = st.columns([1, 1.5, 1])\
    with col_l2:\
        input_email = st.text_input("College Email Address", placeholder="username@stmaryscollege.in").strip().lower()\
        input_password = st.text_input("Password", type="password", placeholder="\'95\'95\'95\'95\'95\'95\'95\'95")\
        \
        if st.button("Sign In", type="primary", use_container_width=True):\
            # Fetch passwords dynamically from Secrets map\
            try:\
...
\
# --- SIDEBAR UTILITIES (Sign Out & Password Management) ---\
with st.sidebar:\
    st.image("https://img.icons8.com/fluent/96/000000/shield.png", width=60)\
    st.markdown(f"**Logged in as:**\\n`\{st.session_state.logged_email\}`")\
    \
    if st.button("\uc0\u55357 \u56594  Secure Sign Out", use_container_width=True):\
        st.session_state.authenticated = False\
        st.session_state.logged_email = ""\
        st.rerun()\
        \
    st.markdown("---")\
    st.subheader("\uc0\u9881 \u65039  Account Settings")\
    with st.expander("\uc0\u55357 \u56593  Change Password"):\
...
st.title("\uc0\u55356 \u57314  St. Mary's Manual Research Logging Desk")\
st.markdown("Fill out your entry matrix fields and submit them cleanly to the Master Sheet ledger.")\
\
st.markdown("---")\
st.subheader("\uc0\u55357 \u56420  Faculty Member Profile")\
col_a, col_b, col_c = st.columns(3)\
with col_a:\
    # Pre-fill the email address text box cleanly based on who logged in\
    form_email_display = st.text_input("Faculty Email Address", value=st.session_state.logged_email, disabled=True)\
with col_b:\
...
with col_d if 'col_d' in locals() else st.columns(1)[0]:\
    form_year = st.selectbox("Select Academic Year", ACADEMIC_YEARS)\
\
st.markdown("---")\
st.subheader("\uc0\u55357 \u56522  Research Matrix Log (Up to 10 Rows)")\
\
...
for i in range(1, 11):\
    ...
    with row_col1:\
        st.markdown(f"<p style='padding-top:25px; text-align:center;'>\{i\}</p>", unsafe_with_html=True)\
    with row_col2:\
...
st.markdown("---")\
\
if st.button("\uc0\u55357 \u56960  Process Batch & Commit Records to Sheet", type="primary"):\
    if not list(form_name.strip()):\
...
\
            for idx, entry in enumerate(row_data_collection):\
                status_text.markdown(f"\uc0\u55357 \u56510  **Processing Entry Row \{entry['sl_no']\}:** `\{entry['title']\}`...")\
                \
...
\
            ...
            sheets_service.spreadsheets().values().update(\
                spreadsheetId=MASTER_SHEET_ID, range=f"'\{form_year\}'!A1", valueInputOption="USER_ENTERED", body=\{'values': final_write_payload\}\
            ).execute()\
\
            status_text.empty()\
            progress_bar.empty()\
            st.success(f"\uc0\u55356 \u57225  Successfully logged all structural matrix rows to the '\{form_year\}' sheet ledger!")\
            st.balloons()\
            \
