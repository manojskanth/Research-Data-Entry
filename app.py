...
def extract_announcements_from_docx(file_bytes):
    ...
    try:
        ...
        # 1. Scan for overall Category & Department context
        ...
        # 2. Table Row Processing (Each row produces exactly one complete card)
        for table in doc.tables:
            ...
            for row_idx, row in enumerate(table.rows):
                ...
                # Skip repeated header row in table
                ...
                # Require a valid title (or derive from link domain) - never render an orphan card!
...
# Perfectly structured card renderer without HTML syntax bugs
...
st.markdown("---")
st.markdown("<div style='text-align: center; color: olive;'>Developed by Research Committee @ St. Mary's College</div>", unsafe_allow_html=True)import streamlit as st
import datetime
from google.oauth2 import service_account
from googleapiclient.discovery import build
from googleapiclient.http import MediaIoBaseUpload, MediaIoBaseDownload
